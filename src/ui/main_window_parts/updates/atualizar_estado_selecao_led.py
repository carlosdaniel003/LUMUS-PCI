...
def atualizar_estado_selecao_led(self, selecao_ativa: bool) -> None:
    ...
    # O editor trabalha sempre nas coordenadas absolutas da imagem original.
    # Ao entrar em Selecionar LEDs, a rotação apenas visual volta a 0° para
    # impedir qualquer ambiguidade entre alças, marquee e posições persistidas.
    if self.selecao_led_ativa:
        definir_rotacao = getattr(
            self,
            "definir_rotacao_visual_principal",
            None,
        )
        if callable(definir_rotacao):
            definir_rotacao(0, notificar=False)
