...
def copiar_led(led: LedSelection) -> LedSelection:
    """Copia a geometria atual; coordenadas normalizadas serão refeitas ao salvar."""
    return LedSelection(
        id=str(led.id),
...
def limitar_deslocamento_rois(
    ...
    """Limita um deslocamento coletivo para nenhuma ROI sair da imagem."""
    itens = list(leds)
    if not itens or largura <= 0 or altura <= 0:
        return 0, 0

    minimo_x = max(int(led.raio) - int(led.centro_x) for led in itens)
    maximo_x = min(
        int(largura) - 1 - int(led.raio) - int(led.centro_x)
        for led in itens
    )
    minimo_y = max(int(led.raio) - int(led.centro_y) for led in itens)
    maximo_y = min(
        int(altura) - 1 - int(led.raio) - int(led.centro_y)
        for led in itens
    )
...
def mover_rois(
    ...
    return [
        LedSelection(
            id=led.id,
            centro_x=led.centro_x + dx,
            centro_y=led.centro_y + dy,
            raio=led.raio,
        )
        for led in itens
    ]
...
def _criar_rois_escaladas(
    ...
    return [
        LedSelection(
            ...
            centro_x=int(round(
                centro_grupo_x
                + (int(led.centro_x) - centro_grupo_x) * escala
            )),
            centro_y=int(round(
                centro_grupo_y
                + (int(led.centro_y) - centro_grupo_y) * escala
            )),
            raio=int(round(int(led.raio) * escala)),
        )
        for led in leds
    ]
...
def escalar_rois_uniformemente(
    ...
    """Escala posições e raios igualmente, preservando o desenho do conjunto."""
    ...
    escala = min(
        escala_maxima,
        max(escala_minima, float(escala_desejada)),
    )
    ...
    # A ampliação pode encostar em uma borda. Busca a maior escala válida sem
    # permitir que uma única ROI saia do frame.
    if escala <= 1.0:
        return origem
...
def ajustar_raios_rois(
    ...
    """Ajusta os raios sem deslocar centros nem ultrapassar as bordas."""
    resultado = []
    for item in leds:
        led = copiar_led(item)
        limite_borda = min(
...
class BulkRoiEditorMixin:
    """Edição de uma ROI ou de todo o conjunto sem remover o fluxo existente."""
    ...
    def _substituir_leds_editaveis(
        self,
        leds: Iterable[LedSelection],
    ) -> None:
    ...
    def _bbox_grupo_imagem(
        self,
        leds: Iterable[LedSelection] | None = None,
    ...
    def _handle_atingido(self, canvas_x: int, canvas_y: int) -> bool:
        if self._roi_editor_selection != "all":
            return False
        for handle_x, handle_y in self._handles_grupo_canvas():
            if (
                abs(float(canvas_x) - handle_x) <= HANDLE_HIT_CANVAS_PX
                and abs(float(canvas_y) - handle_y) <= HANDLE_HIT_CANVAS_PX
            ):
                return True
        return False
    ...
    def _led_atingido(
        ...
        escala = max(0.000001, float(getattr(self.view, "escala_exibicao", 1.0)))
        tolerancia = max(2, int(round(7.0 / escala)))
        melhor = None
        ...
        for led in self._leds_editaveis():
            ...
            limite = int(led.raio) + tolerancia
            if distancia <= limite * limite:
    ...
    def _selecionar_todas_rois(self, mensagem: bool = True) -> bool:
        ...
        if mensagem:
            self.view.atualizar_status(
                f"{len(leds)} ROIs selecionadas. Arraste o quadro para mover; "
                "arraste um canto para escalar; roda do mouse ajusta os raios; "
    ...
    def _selecionar_roi_individual(self, led: LedSelection) -> None:
        ...
        self._desenhar_overlay_editor_roi()
        self.view.atualizar_status(
            f"{led.id} selecionado. Arraste para mover; roda do mouse ajusta o raio; "
            "Delete apaga somente esta ROI."
        )
    ...
    def _desenhar_overlay_editor_roi(self) -> None:
        ...
        if self._roi_editor_selection == "single":
            led = next(
                (item for item in leds if str(item.id) == self._roi_editor_single_id),
                None,
            )
            if led is None:
                return
        ...
        for hx, hy in self._handles_grupo_canvas():
            canvas.create_rectangle(
                hx - HANDLE_SIZE_CANVAS_PX,
                hy - HANDLE_SIZE_CANVAS_PX,
                hx + HANDLE_SIZE_CANVAS_PX,
                hy + HANDLE_SIZE_CANVAS_PX,
                fill="#FBBF24",
                outline="#111827",
        ...
        canvas.create_text(
            (cx1 + cx2) / 2.0,
            max(12, cy1 - 14),
            text=f"TODAS AS ROIs ({len(leds)})",
            fill="#FBBF24",
            font=("DejaVu Sans", 8, "bold"),
            tags=ROI_EDITOR_TAG,
        )

    def _iniciar_arrasto(
        self,
        modo: str,
        imagem_x: int,
        imagem_y: int,
        ...
        if modo == "scale_all":
            ...
            self._roi_editor_scale_distance = max(
                1.0,
                math.hypot(imagem_x - centro_x, imagem_y - centro_y),
            )
    ...
    def evento_clique_esquerdo(self, evento) -> str | None:
        ...
        if (
            self._roi_editor_selection == "all"
            and self._ponto_dentro_grupo(imagem_x, imagem_y)
        ):
            self._cancelar_long_press_roi()
        ...
        # Clique curto continua criando uma ROI como antes. Clique segurado ou
        # arrasto iniciado no vazio entra no modo coletivo.
        ...
        self._roi_editor_long_press_after_id = self.root.after(
            LONG_PRESS_MS,
            lambda: self._ativar_selecao_total_por_long_press(
                imagem_x,
                imagem_y,
            ),
        )
    ...
    def _ativar_selecao_total_por_long_press(
        self,
        imagem_x: int,
        imagem_y: int,
    ) -> None:
    ...
    def _evento_arrastar_roi(self, evento) -> str | None:
        ...
        if self._roi_editor_drag_mode == "pending_all":
            ...
            distancia = math.hypot(
                evento.x - origem_canvas[0],
                evento.y - origem_canvas[1],
            )
        ...
        if self._roi_editor_drag_mode == "move_all":
            ...
            indice = next(
                (
                    idx
                    for idx, led in enumerate(novos)
    ...
    def _evento_soltar_roi(self, evento) -> str | None:
        ...
        if modo == "pending_all":
            ...
            self._roi_editor_selection = None
            self._roi_editor_single_id = None
            # Clique curto no vazio mantém a funcionalidade original de criar ROI.
        ...
        if modo in {"move_all", "move_single", "scale_all"}:
            ...
            self.view.atualizar_status(
                f"{acao}. As demais funções de seleção e análise foram mantidas."
            )
            return "break"
        return None
    ...
    def _evento_apagar_roi(self, _evento=None) -> str | None:
        ...
        self._atualizar_pos_edicao_roi()
        self.view.atualizar_status(
            f"{removidos} ROI(s) removida(s) da seleção atual. "
            "A configuração salva só muda quando o fluxo existente de salvar LEDs é usado."
        )
        return "break"
    ...
    def _evento_cancelar_selecao_roi(self, _evento=None) -> str | None:
        ...
        self.view.atualizar_status(
            "edição de ROI desmarcada; as posições foram mantidas."
        )
        return "break"
    ...
    def iniciar_selecao_led(self) -> None:
        ...
        if ativando_camera:
            ...
            if atuais and not getattr(self, "leds_manuais_camera", []):
                # Permite editar em massa as guias carregadas, sem apagar o JSON.
                self.leds_manuais_camera = [copiar_led(led) for led in atuais]
        self._resetar_editor_roi()
        super().iniciar_selecao_led()
        if self._modo_edicao_roi_ativo():
            self.view.atualizar_status(
                "seleção de LEDs ativa. Clique para adicionar; clique numa ROI para "
                "editar individualmente; clique e segure ou arraste no vazio para "
                "selecionar todas; Ctrl+A também seleciona o conjunto."
            )
